...
public class GroupDetailsActivity extends BaseGestureActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle(group.getGroupName() + "(" + group.getAffiliationsCount() + st);
                            ...
                            public void run() {
                                try {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        public void run() {
                                            toolbar.setTitle(returnData + "(" + group.getAffiliationsCount() + st);
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            To.show(getResources().getString(R.string.Exit_the_group_chat_failure) + " " + e.getMessage());
                        }
                    });
                }
            }
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            refreshMembers();
                            toolbar.setTitle(group.getGroupName() + "(" + group.getAffiliationsCount() + ")");
                            progressDialog.dismiss();
                        }
                    });
                } catch (final Exception e) {
        ...
        switch (v.getId()) {
            case R.id.rl_switch_block_groupmsg: // 屏蔽群组
                if (iv_switch_block_groupmsg.getVisibility() == View.VISIBLE) {
                    L.d("change to unblock group msg");
                    if (progressDialog == null) {
                        progressDialog = new ProgressDialog(GroupDetailsActivity.this);
                        progressDialog.setCanceledOnTouchOutside(false);
                    }
                ...
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            // update block
                            L.d("group msg is blocked:" + group.getMsgBlocked());
                    ...
                    public void onClick(View v) {
                        if (isInDeleteMode) {
                            ...
                            if (!NetUtils.hasNetwork(getApplicationContext())) {
                                To.show(getString(R.string.network_unavailable));
                                return;
                            }
